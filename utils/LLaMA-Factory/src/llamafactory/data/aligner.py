...
def convert_alpaca(
    ...
    for i in range(len(examples[dataset_attr.prompt])):
        ...
        if dataset_attr.kto_tag and isinstance(examples[dataset_attr.kto_tag][i], bool):  # kto example
            ...
            dataset_attr.add_double_thought
            and dataset_attr.has_double_task
            and isinstance(examples[dataset_attr.first_thought][i], str)
            and isinstance(examples[dataset_attr.second_thought][i], str)
            and isinstance(examples[dataset_attr.response][i], str)
        ): # double think example
            response = [
                {"role": Role.THOUGHT_ST.value, "content": examples[dataset_attr.first_thought][i]},
            ...
            and not dataset_attr.has_subtask ## Perform QTA directly without subtask prompt
            and isinstance(examples[dataset_attr.thought][i], str)
            and isinstance(examples[dataset_attr.response][i], str)
        ): 
            response = [
                {"role": Role.THOUGHT.value, "content": examples[dataset_attr.thought][i]},
                {"role": Role.ASSISTANT.value, "content": examples[dataset_attr.response][i]}
            ]
        elif (
            dataset_attr.add_thought
            and dataset_attr.has_subtask ## Perform QTA with subtask prompt(like wos)
            and isinstance(examples[dataset_attr.thought][i], str)
            and isinstance(examples[dataset_attr.subtask][i], str)
            and isinstance(examples[dataset_attr.response][i], str)
        ): 
            response = [
                {"role": Role.THOUGHT.value, "content": examples[dataset_attr.thought][i]},
                {"role": Role.SUBTASK.value, "content": examples[dataset_attr.subtask][i]},
                {"role": Role.ASSISTANT.value, "content": examples[dataset_attr.response][i]}
            ]
        ...
        outputs["system"].append(examples[dataset_attr.prompt][i] if dataset_attr.prompt else "")
